...
export default class implements Command {
  public name = ['play']
  public description = 'Play a song or playlist from url or name'
  ...
  public async execute(client: Manager, handler: CommandHandler) {
    ...
    // Set the bot's presence to the currently playing song
    try {
      client.user?.setActivity(track.title, { type: ActivityType.Listening })
    } catch (error) {
      client.logger.error('PresenceUpdate', `Failed to set activity: ${error}`)
    }

    // Set the voice channel status to the currently playing song
    const voiceChannel = handler.member?.voice.channel
    if (voiceChannel) {
      try {
        const songTitle = track.title
        const status = `${songTitle}`
        // Truncate the status if it's longer than 100 characters.
        const truncatedStatus = status.length > 100 ? status.substring(0, 97) + '...' : status
        if ('setStatus' in voiceChannel) {
          await (voiceChannel as any).setStatus(truncatedStatus)
        }
      } catch (error) {
        client.logger.error('VoiceChannelStatus', `Failed to set voice channel status: ${error}`)
      }
    }
