    ...
    codeSplit.forEach((chunk, ci) => {
        if (chunk.startsWith('`') && chunk.endsWith('`')) {
            parts.push(
                <code
                    key={`c-${ci}`}
                    className='rounded bg-vaded-surface-elevated border border-vaded-border-soft px-1.5 py-0.5 font-mono text-[0.84em] text-vaded-text-strong'
                >
                    {chunk.slice(1, -1)}
                </code>,
            )
...
const sectionStyles: Record<string, string> = {
    Added: 'text-vaded-brand',
    Changed: 'text-amber-400',
    Fixed: 'text-emerald-400',
    Internal: 'text-vaded-text-muted',
    ...
    return (
        <div className='min-h-screen bg-vaded-surface-canvas text-white'>
            ...
            <div className='mx-auto grid max-w-7xl gap-0 px-0 md:px-6 lg:grid-cols-[16rem_minmax(0,1fr)_14rem]'>
                <aside
                    className={`${sidebarOpen ? 'block' : 'hidden'} lg:block border-r border-vaded-border-soft bg-vaded-surface-canvas`}
                            ...
                            {entries.map((e) => {
                                ...
                                return (
                                    <li key={e.version}>
                                        <a
                                            href={`#v-${e.version}`}
                                            className={`flex w-full items-center justify-between gap-2 rounded-md px-2.5 py-1.5 text-sm transition-colors ${active ? 'bg-vaded-surface-panel text-vaded-text-strong shadow-[inset_2px_0_0_var(--color-vaded-brand)]' : 'text-vaded-text-body hover:bg-vaded-surface-panel hover:text-vaded-text-strong'}`}
                                        >
                                            <span className='font-mono'>
                                                {e.version}
                                            </span>
                                            {e.date ? (
                                                <span className='font-mono text-[10px] text-vaded-text-muted'>
                                                    {e.date.slice(0, 7)}
                                                </span>
                                            ) : null}
                                        </a>
                ...
                <main className='min-w-0 px-4 py-10 md:px-8 md:py-14 lg:px-10'>
                    <article className='mx-auto max-w-2xl'>
                        <header className='mb-10 border-b border-vaded-border-soft pb-6'>
                            ...
                            <h1 className='text-3xl font-bold tracking-tight text-vaded-text-strong md:text-4xl'>
                                Changelog
                            </h1>
                            <p className='mt-3 text-sm text-vaded-text-body'>
                        ...
                        <div className='relative'>
                            <div
                                aria-hidden
                                className='absolute left-2 top-2 bottom-2 w-px bg-vaded-border-soft'
                            />
                            <div className='space-y-12'>
                                {entries.map((entry) => (
                                    ...
                                    >
                                        <span
                                            aria-hidden
                                            className='absolute left-[3px] top-2.5 h-2.5 w-2.5 rounded-full bg-vaded-brand ring-4 ring-vaded-surface-canvas'
                                        />
                                        <div className='flex flex-wrap items-baseline gap-x-3 gap-y-1'>
                                            <h2 className='font-mono text-xl font-semibold text-vaded-text-strong'>
                                                v{entry.version}
                                            </h2>
                                            {entry.date ? (
                                                <span className='font-mono text-xs text-vaded-text-muted'>
                                            ...
                                            {entry.sections.map((section) =>
                                                ...
                                                0 ? null : (
                                                    <div key={section.heading}>
                                                        <h3
                                                            className={`mb-2 font-mono text-[11px] uppercase tracking-[0.22em] ${sectionStyles[section.heading] ?? 'text-vaded-text-muted'}`}
                                                        >
                                                            {section.heading}
                                                        </h3>
                                                        <ul className='space-y-1.5 text-sm leading-relaxed text-vaded-text-body'>
                                                            {section.items.map(
                                                                (item, i) => (
                                                                    <li
                                                                        key={i}
                                                                    ...
                                                                    >
                                                                        <span className='mt-2 inline-block h-1 w-1 shrink-0 rounded-full bg-vaded-text-muted' />
                                                                        <span>
                                                                            {renderInlineMd(
                                                                                item,
                                                                            )}
