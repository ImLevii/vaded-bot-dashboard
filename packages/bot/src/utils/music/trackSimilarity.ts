...
export function calculateTrackSimilarity(trackA: Track, trackB: Track): number {
    ...
    similarity +=
        calculateDurationSimilarity(trackA.duration, trackB.duration) * 0.2
    if (trackA.url === trackB.url) similarity += 0.1
    return similarity
}

export function calculateDurationSimilarity(
    durationA: number | string,
    durationB: number | string,
): number {
    const numA =
        typeof durationA === 'number'
            ? durationA
            : parseInt(durationA.toString())
    const numB =
        typeof durationB === 'number'
            ? durationB
            : parseInt(durationB.toString())
...
export function calculateTrackQuality(track: Track): number {
    ...
    // Lavalink tracks carry no view-count field (unlike discord-player's
    // extractor-supplied one), so that quality signal is dropped.
