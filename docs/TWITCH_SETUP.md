...
Vaded Gaming can notify a Discord channel when a configured Twitch streamer goes live, using Twitch EventSub over WebSocket.
...
3. Fill the form:
    - **Name**: Application name (e.g. `Vaded Gaming`), 3–100 characters. Shown to users when they authorize the app.
    ...
    - **Client type**: Select **Confidential** for a server-side app like Vaded Gaming (credentials stay on the server). Use **Public** only for client-side or mobile apps where the secret cannot be kept private.
...
You need one user (e.g. the bot owner) to authorize the app so Vaded Gaming can create EventSub subscriptions and call the Helix API.
...
- On bot ready, if Twitch env is set, Vaded Gaming connects to `wss://eventsub.wss.twitch.tv/ws`, receives a session id, and creates `stream.online` subscriptions for each distinct Twitch user id stored in the database.
