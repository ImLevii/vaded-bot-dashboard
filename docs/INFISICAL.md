...
Vaded Gaming can load environment variables from [Infisical](https://infisical.com) when the Infisical client is configured. This keeps secrets out of `.env` in production and centralizes them in Infisical.

## Configuration checklist

| Step | Where             | What                                                                                                                                                 |
| ---- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Infisical UI      | Create project (e.g. Vaded Gaming), add environments (`dev`, `prod`), create a Machine Identity, copy **Client ID** and **Client Secret**.                  |
...
## Configure .env for Vaded Gaming (Infisical)

To have Vaded Gaming load secrets from Infisical at runtime, add these to your **local** `.env` (do not commit the secret):

| Variable                  | Where to get it                                                                                 |
| ------------------------- | ----------------------------------------------------------------------------------------------- |
| `INFISICAL_CLIENT_ID`     | Infisical → Vaded Gaming → Access Control → Project Machine Identities → your identity → **Client ID** |
| `INFISICAL_CLIENT_SECRET` | Same screen → Client Secrets (copy once; store securely)                                        |
| `INFISICAL_PROJECT_ID`    | Infisical → Vaded Gaming → Settings → Project ID, or from MCP `list-projects` after MCP is configured  |
...
INFISICAL_PROJECT_ID=<vaded-project-id>
INFISICAL_ENV=dev
```
...
To use Infisical MCP tools (e.g. list projects, list/create secrets) from Cursor, the Infisical MCP server must be authenticated with a Machine Identity **that has access to the Vaded Gaming project**:

1. In Infisical: **Vaded Gaming** → **Access Control** → **Project Machine Identities** → add or select an identity (e.g. **MCP**) → grant it **Read** and **Write** (or **Admin**) on secrets → copy **Client ID** and create/copy a **Client Secret**.
...
3. After that, MCP tools like `list-projects` (with `type: "secret-manager"`) and `list-secrets` / `create-secret` (with Vaded Gaming `projectId`, `environmentSlug`) work for Vaded Gaming. If you get **403** on Vaded Gaming, the identity used by MCP is not attached to Vaded Gaming—attach it in step 1. If you get **422** on `list-projects`, use `type: "secret-manager"` (not `"all"`).

Do not paste real client secrets into chat or into the repo; keep them in Cursor MCP config or in local `.env` only.

### Vaded Gaming project (Infisical)

Vaded Gaming project overview: [Infisical Vaded Gaming](https://app.infisical.com/organizations/1843fc0a-2cce-46ca-a6d6-a31929356381/projects/secret-management/fdc73498-94a7-46ba-9ecc-b64956e45af3/overview). Use **Project ID** `fdc73498-94a7-46ba-9ecc-b64956e45af3` for MCP calls and for `INFISICAL_PROJECT_ID` in `.env`.

### Run from Infisical (dev)

Dev secrets are synced from `.env` to Vaded Gaming → dev. To run the bot/backend using Infisical instead of local `.env`, add to `.env` (keep `INFISICAL_CLIENT_SECRET` out of version control):

```env
INFISICAL_PROJECT_ID=fdc73498-94a7-46ba-9ecc-b64956e45af3
INFISICAL_ENV=dev
INFISICAL_CLIENT_ID=<from Vaded Gaming → Access Control → Machine Identities>
...
With the Infisical MCP server configured with a Machine Identity that has access to Vaded Gaming:

- **List projects**: `list-projects` with `type: "secret-manager"` (API accepts `secret-manager`, `cert-manager`, `kms`, `ssh`, etc.; not `"all"`).
- **List secrets**: `list-secrets` with `projectId` (e.g. Vaded Gaming `fdc73498-94a7-46ba-9ecc-b64956e45af3`) and `environmentSlug` (e.g. `dev`).
