...
1. Confirm `DEPLOY_WEBHOOK_URL` points to an existing DNS host (Vaded Gaming):
    - `gh secret set DEPLOY_WEBHOOK_URL --body "https://vaded.gg/webhook/deploy"`
2. Ensure Cloudflare DNS has `vaded.gg` published to the active tunnel.
...
4. Confirm public probes recover:
    - `https://vaded.gg/api/health` -> `200`
    - `https://vaded.gg/api/health/auth-config` -> `200`
    - `https://vaded.gg/api/auth/discord` -> `302`
