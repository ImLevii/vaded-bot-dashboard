...
3. The homelab builds the branch's images locally (~2-3 min) and brings up the
   `vaded-staging` stack; every subsequent push to the labeled PR redeploys.
...
- `/home/luk-server/vaded-staging` — the staging git checkout (the deploy script
  checks out the target ref here and builds from it).
- `/home/luk-server/vaded-staging/.env.staging` — staging env: prod values with
  `WEBAPP_*` pointed at the staging domains, a distinct `WEBAPP_SESSION_SECRET`,
  and `NGINX_PORT=8093`.
- `~/.cloudflared/config-vaded.yml` — ingress rules routing the two staging
  hostnames to `http://100.95.204.103:8093`.
- Cloudflare DNS — `vaded-staging` + `vaded-staging-api` CNAMEs → the tunnel.
