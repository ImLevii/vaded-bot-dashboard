...
Accepted (decision). Triggered by Snyk audit on 2026-05-15 surfacing 3 critical + 11 high CVEs in `vaded-nginx` and `vaded-frontend` images that the existing CI security stack had missed.
...
Concrete impact: Snyk's GitHub App (already installed, scanning daily) surfaced 3 critical + 11 high Alpine 3.21 OS package CVEs in the nginx-based images. None of these were visible in Vaded Gaming's CI; they only appeared on the Snyk dashboard. PR #881 fixed them via `apk upgrade --no-cache`.
...
| **C. Extend Trivy per-repo + keep Snyk's GitHub App** (chosen) | New `trivy-image` step in Vaded Gaming's `docker-publish.yml`; no org changes; no Snyk wiring | OS + lang deps in CI + Snyk dashboard async monitoring | Free                                                                  | Vaded Gaming only; opt-in template for other repos later     |

### Critique (Phase 2)

The `critic` agent flagged three concerns, all accepted:

1. **Image-scan placement** — should NOT be mandated in the org reusable because caller repos are heterogeneous (some don't build images). **Accepted:** Vaded Gaming-specific in `docker-publish.yml`; opt-in template for others.
...
3. **Snyk free-tier risk** — Snyk has changed free-tier limits before. If the GitHub App's free monitoring sunsets, Vaded Gaming loses dashboard visibility. **Accepted:** quarterly check as a revisit trigger; Trivy image-scan becomes the blocking fallback if Snyk app monitoring disappears.
...
1. **Add `trivy-action` image-scan to Vaded Gaming's `docker-publish.yml`** — runs against newly-built images before push, so the scan cost is tied to actual publish events (not every PR).
