...
deprecations killed Related Artists/Recommendations/Audio-Features for new apps (Vaded Gaming
already routes "related" through a Last.fm `getSimilar` + name-lookup workaround). If
Vaded Gaming's Spotify app is in **Development Mode** (5-user cap, lower limits), 429s — not
...
**GATE RESULT (2026-06-01, Sentry scan of project `vaded`):** ZERO Spotify 429s. Backend
...
1. **Postgres cache table for Tier 2** (`SpotifyArtistCache`) — _rejected for now._ The
   ...
   in-memory is simpler for a single-instance hobby app. **Revisit if Vaded Gaming scales out.**
...
- Vaded Gaming scales to **multiple instances** → move per-user Tier 2 cache to **Postgres** (alt 1).
