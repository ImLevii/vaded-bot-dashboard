...
- `docs/FFMPEG_USAGE.md` (839 lines): comprehensive ffmpeg reference — duplicates upstream docs; the only Vaded Gaming-specific content is three sentences in the introduction.
...
- `.context/vaded-rag-context.md` (441 lines): RAG indexer output.
...
> What's our policy for tracking AI-generated or auto-generated documentation in the Vaded Gaming repo?
...
1. **Knowledge loss risk** for two duplicate-shaped docs (FFMPEG_USAGE, DISCORD_PLAYER_GUIDE). **Validated:** grep confirmed Vaded Gaming-specific content is limited to boilerplate intros ("Comprehensive guide for using X in Vaded Gaming"). The substantive content duplicates upstream docs.
...
4. **Policy** for what may live in the tracked docs tree:
    ...
    - **Borderline (default to runbook):** if an AI tool surfaces a real Vaded Gaming-specific gotcha, convert it to a `docs/runbooks/` entry with explicit ownership and prune it from the AI output, rather than committing the AI output verbatim.
...
- An AI tool reliably generates Vaded Gaming-specific scaffolding (not duplicate-of-upstream) that solves a real onboarding gap. Then escalate that specific output to a runbook rather than relax the policy.
