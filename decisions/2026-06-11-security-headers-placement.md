...
- The SPA users actually hit is served by **Vercel** (`vercel.json`, Vite build), with `/api/:path*` rewritten to `https://api.vadedgaming.com`. nginx-only headers would miss the primary user-facing origin entirely.
